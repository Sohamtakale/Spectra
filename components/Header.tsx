...
export default function Header() {
    ...
    const [rank, setRank] = useState<{ score: number, title: string }>({ score: 0, title: 'Novice' });
    const supabase = createClient();
    const router = useRouter();

    const fetchRank = async (userId: string) => {
        const { data } = await supabase.from('users').select('trust_rank').eq('id', userId).single();
        if (data) {
            const score = data.trust_rank || 0;
            let title = 'Novice';
            if (score >= 50) title = 'Truth Seeker';
            else if (score >= 10) title = 'Fact Checker';
            setRank({ score, title });
        }
    };
    ...
    useEffect(() => {
        ...
        const setupRealtime = async (userId: string) => {
            channel = supabase
                ...
                .on(
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    (payload: any) => {
                        const newScore = payload.new.trust_rank;
                        let title = 'Novice';
                        if (newScore >= 50) title = 'Truth Seeker';
                        else if (newScore >= 10) title = 'Fact Checker';
                        setRank({ score: newScore, title });
                    }
                )
                .subscribe();
        };
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
            ...
            if (session?.user) {
                fetchRank(session.user.id);
                // Re-setup realtime if users change (unlikely in single session but good practice)
            ...
            < div className="container flex h-20 max-w-screen-2xl items-center justify-between">
                ...
                < div className="flex flex-1 items-center justify-center gap-8 md:justify-end md:gap-4">
                    < nav className="hidden md:flex gap-6 text-sm font-medium tracking-wide uppercase text-muted-foreground">
                        ...
                        <Link href="/media-literacy" className="hover:text-foreground transition-colors text-primary font-bold">Education</Link>
                    </nav >
                        ...
                        {user ? (
                            <div className="flex items-center gap-4">
                                ...
                                <div className="hidden md:flex flex-col items-end">
                                    <span className="text-[0.6rem] uppercase tracking-widest text-muted-foreground font-semibold">Investigator Rank</span>
                                    <div className="flex items-center gap-2">
                                        <div className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" />
                                        <span className="text-xs font-bold font-mono text-primary">
                                            LVL {Math.floor(rank.score / 10) + 1}: {rank.title}
                                        </span>
                                    </div>
                                </div>
